...
#Task1
print("Task1")
payMatrixA=np.array([[0.25,0.35,0.4], #платежная матрица
                  [0.7,0.2,0.3],
                  [0.35,0.85,0.2],
                  [0.8,0.1,0.35]])
qmatrixA=np.array([0.5,0.3,0.2]) #вероятности реализации каждой обстановки

print(findBenefitInPayMatrix(payMatrixA,qmatrixA)) #сумма произведений вероятностей и каждого значения стратегий Аi

#Task2
print("Task2")
payMatrixB=np.array([[280,140,210,245],#платежная матрица
                  [420,560,140,280],
                  [245,315,350,490]])
...
matrixOfProbabilities=np.array([0.25,0.15,0.2,0.4]) #матрица вероятности состояний природой
costs=1.1 #затраты на проведение эксперимента для выяснения условий оперции
# Необходимо определить целесообразность проведения эксперимента в предположении,
# что он позволяет точно определить состояние природы  , при  котором будет осуществляться операция.
