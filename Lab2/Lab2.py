...
# Описываем переменные (коэффициенты функции ограничений)
x = {i: LpVariable(name=f"x{i}", lowBound=0) for i in range(0, 3)}
# Добавляем ограничения
model += (14*x[0] + 15*x[1] + 33*x[2] >= 1, "Первый")
model += (20*x[0] + 11*x[1] + 9 *x[2] >= 1, "Второй")
model += (32*x[0] + 19*x[1] + 16*x[2] >= 1, "Третий")
model += (8*x[0]  + 37*x[1] + 34*x[2] >= 1, "Четвертый")
# Описываем цель
model +=  x[0] +  x[1] +x[2]
# Решаем задачу оптимизации
status = model.solve()
# Выводим результаты решения (objective - значение целевой функции)
...
print("рыночная доля 1 предприятия:",53+priceOfGameOne+minElement,"%")
print("рыночная доля 2 предприятия:",47-priceOfGameOne+minElement,"%")
print("Средний выигрыш первого игрока составляет:",np.dot(np.dot(pFirstPlayer,matrixTask2),qSecondPlayer))
#Task3
print("Task3")
...
#Point1
print("Point1")
print("Средний выигрыш первого игрока при смешанных стратегиях второго игрока составляет:",np.dot(np.dot(pFirstPlayer,matrixTask3),qSecondPlayer))
#Point2
print("Point2")
qSecondPlayer=np.array([1,0,0])
print("Средний выигрыш первого игрока, когда игрок В применяет одну из чистых стратегий :",np.dot(np.dot(pFirstPlayer,matrixTask3),qSecondPlayer))
